...
def setup_vector_db():
    ...
    df['prompt_and_story'] = df['prompt'] + ' ' + df['text']
    ...
    assert 'prompt' in df.columns, "'prompt' column not found in DataFrame"

    loader = DataFrameLoader(df.sample(5), page_content_column="prompt_and_story")
